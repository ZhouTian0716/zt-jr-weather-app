...
const ThemeToggle = () => {
  ...
  const getCurrentTheme = () => {
    // 1st Check, getting the system default theme first
    let theme = window.matchMedia("(prefers-color-scheme: dark)").matches
      ? "dark"
      : "light";
    // 2nd Check, if user set the theme before
  ...
  const lightThemeClass = styles.c_theme;
  const darkThemeClass = `${styles.c_theme} ${styles.c_theme__active}`;

  return (
    <button
      className={isThemeDark ? (darkThemeClass):(lightThemeClass)}
      onClick={handleThemeChange}
    >
      <div className={styles.c_theme__track}></div>
      <div className={styles.c_theme__knob}>
        <div className={`${styles.c_theme__icon_wrapper} ${styles.c_theme__icon_wrapper__light}`}>
          <svg
            className={styles.c_theme__icon}
            viewBox="0 0 16 16"
        ...
        <div className={`${styles.c_theme__icon_wrapper} ${styles.c_theme__icon_wrapper__dark}`}>
          <svg
            className={styles.c_theme__icon}
            viewBox="0 0 17 16"
            xmlns="http://www.w3.org/2000/svg"
